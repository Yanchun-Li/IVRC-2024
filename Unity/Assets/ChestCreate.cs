...
public class ChestCreate : MonoBehaviourPunCallbacks
{
    ...
    public void MakeTresure(){
        ...
        for (int i=0;i<number;i++){
            ...
            GameObject chest1 = PhotonNetwork.Instantiate(chestParent,localposition+originalPosition,Quaternion.identity);
            GameObject chest2 =PhotonNetwork.Instantiate(chestParent,localposition+CopyoriginalPosition,Quaternion.identity);
            chests.Add(chest1);
            chests.Add(chest2);
        }
        createTreasure = true;
